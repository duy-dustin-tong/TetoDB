...
namespace tetodb {
    ...
    enum class CompType { EQUAL, NOT_EQUAL, LESS_THAN, GREATER_THAN, LESS_THAN_OR_EQUAL, GREATER_THAN_OR_EQUAL };

    class ComparisonExpression : public AbstractExpression {
    private:
        // Helper to safely move unique_ptrs into the base class vector
        static std::vector<std::unique_ptr<AbstractExpression>> MakeChildren(
    ...
    public:
        ComparisonExpression(CompType comp_type,
            ...
            : AbstractExpression(MakeChildren(std::move(left), std::move(right))), comp_type_(comp_type) {
        }
        ...
        Value EvaluateJoin(const Tuple* left_tuple, const Schema* left_schema,
            ...
            Value lhs = GetChildAt(0)->EvaluateJoin(left_tuple, left_schema, right_tuple, right_schema, params);
            Value rhs = GetChildAt(1)->EvaluateJoin(left_tuple, left_schema, right_tuple, right_schema, params);
            return PerformComputation(lhs, rhs);
        }
    ...
    private:
        Value PerformComputation(const Value& lhs, const Value& rhs) const {
            bool result = false;
            switch (comp_type_) {
            case CompType::EQUAL:                 result = lhs.CompareEquals(rhs); break;
            case CompType::NOT_EQUAL:             result = lhs.CompareNotEquals(rhs); break;
            case CompType::LESS_THAN:             result = lhs.CompareLessThan(rhs); break;
            case CompType::GREATER_THAN:          result = lhs.CompareGreaterThan(rhs); break;
                // Safe Fallbacks using logical inversion!
            case CompType::LESS_THAN_OR_EQUAL:    result = !lhs.CompareGreaterThan(rhs); break;
            case CompType::GREATER_THAN_OR_EQUAL: result = !lhs.CompareLessThan(rhs); break;
