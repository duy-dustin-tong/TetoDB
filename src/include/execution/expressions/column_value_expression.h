...
public:
  // ADDED: tuple_idx. 0 = Left Tuple, 1 = Right Tuple. Defaults to 0 for
  // SeqScans.
  ...
  EvaluateJoin(const Tuple *left_tuple, const Schema *left_schema,
               ...
               const std::vector<Value> *params = nullptr) const override {
    if (tuple_idx_ == 0)
      return left_tuple->GetValue(left_schema, col_idx_);
    else
      return right_tuple->GetValue(right_schema, col_idx_);
  }
