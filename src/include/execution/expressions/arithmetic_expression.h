...
namespace tetodb {
    ...
    public:
        ArithmeticExpression(ArithType arith_type,
            ...
            : AbstractExpression(MakeChildren(std::move(left), std::move(right))), arith_type_(arith_type) {
        }
    ...
    private:
        Value PerformComputation(const Value& lhs, const Value& rhs) const {
            switch (arith_type_) {
            case ArithType::ADD:      return lhs.Add(rhs);
            case ArithType::SUBTRACT: return lhs.Subtract(rhs);
            case ArithType::MULTIPLY: return lhs.Multiply(rhs);
            case ArithType::DIVIDE:   return lhs.Divide(rhs);
            default: throw std::runtime_error("Unknown Arithmetic Type!");
            }
        }
