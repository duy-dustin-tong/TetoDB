...
private:
  ...
  void SiftDown(size_t i) {
    ...
    while (true) {
      ...
      if (left < size && heap_[left].free_space > heap_[largest].free_space)
        largest = left;
      if (right < size && heap_[right].free_space > heap_[largest].free_space)
        largest = right;
...
public:
  ...
  page_id_t GetBestPage(uint32_t required_space) {
    if (heap_.empty() || heap_[0].free_space < required_space)
...
class TableHeap {
public:
  TableHeap(BufferPoolManager *bpm, LogManager *log_manager = nullptr,
