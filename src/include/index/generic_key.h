...
namespace tetodb {
    ...
    public:
        ...
        inline int operator()(const GenericKey<KeySize>& lhs, const GenericKey<KeySize>& rhs) const {
            ...
            case TypeId::BOOLEAN:
            case TypeId::TINYINT: {
                int8_t l = *reinterpret_cast<const int8_t*>(lhs.data_);
                int8_t r = *reinterpret_cast<const int8_t*>(rhs.data_);
                return l - r;
            }
            case TypeId::SMALLINT: {
                int16_t l = *reinterpret_cast<const int16_t*>(lhs.data_);
                int16_t r = *reinterpret_cast<const int16_t*>(rhs.data_);
                return l - r;
            }
            case TypeId::INTEGER: {
                int32_t l = *reinterpret_cast<const int32_t*>(lhs.data_);
                int32_t r = *reinterpret_cast<const int32_t*>(rhs.data_);
                return l - r;
            }
            case TypeId::BIGINT: {
                int64_t l = *reinterpret_cast<const int64_t*>(lhs.data_);
                int64_t r = *reinterpret_cast<const int64_t*>(rhs.data_);
                if (l < r) return -1;
                if (l > r) return 1;
                return 0;
            }
            case TypeId::DECIMAL: {
                double l = *reinterpret_cast<const double*>(lhs.data_);
                double r = *reinterpret_cast<const double*>(rhs.data_);
                if (l < r) return -1;
                if (l > r) return 1;
                return 0;
            }
