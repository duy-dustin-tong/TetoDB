...
#define INDEXITERATOR_TYPE IndexIterator<KeyType, ValueType, KeyComparator>
    ...
    INDEX_TEMPLATE_ARGUMENTS
        ...
        public:
            // Constructor
            // The page passed in here is already PINNED and READ-LATCHED by the caller (BPlusTree::Begin)
            ...
            inline const MappingType& operator*() {
                return leaf_->ItemAt(index_);
            }
            ...
            inline bool operator==(const IndexIterator& itr) const {
                if (leaf_ == nullptr && itr.leaf_ == nullptr) return true;
                if (leaf_ == nullptr || itr.leaf_ == nullptr) return false;
                return leaf_->GetPageId() == itr.leaf_->GetPageId() && index_ == itr.index_;
            }

            inline bool operator!=(const IndexIterator& itr) const { return !(*this == itr); }
