...
#define BPLUSTREE_INDEX_TYPE BPlusTreeIndex<KeyType, ValueType, KeyComparator>
    ...
    INDEX_TEMPLATE_ARGUMENTS
        ...
        public:
            BPlusTreeIndex(std::string name, BufferPoolManager* bpm, const KeyComparator& comparator,
                uint32_t leaf_max, uint32_t internal_max, const Schema* key_schema, page_id_t root_page_id = INVALID_PAGE_ID)
                : name_(std::move(name)), key_schema_(key_schema) {
            ...
            void InsertEntry(const Tuple& key_tuple, RID rid, Transaction* txn) override {
                KeyType index_key;
                // Extract the Value from our isolated key tuple and use your existing method!
                index_key.SetFromValue(key_tuple.GetValue(key_schema_, 0));
            ...
            void ScanKey(const Tuple& key_tuple, std::vector<RID>* result, Transaction* txn) override {
                KeyType index_key;
                index_key.SetFromValue(key_tuple.GetValue(key_schema_, 0));
            ...
            const Schema* GetKeySchema() const override { return key_schema_; }
