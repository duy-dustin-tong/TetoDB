...
namespace tetodb {
    ...
    public:
        ...
        // 5. Strings (Varchar)
        Value(TypeId type, std::string s) : type_id_(type), is_null_(false), str_value_(std::move(s)) {}

        Value(TypeId type, const char* s) : type_id_(type), is_null_(false), str_value_(s) {}

        // --- Copy / Move Semantics ---

        // CRITICAL FIX: is_null_ added to the initialization list
        Value(const Value& other) : type_id_(other.type_id_), is_null_(other.is_null_), str_value_(other.str_value_) {
            val_ = other.val_;
        }
