...
namespace tetodb {
    ...
    void TransactionManager::Commit(Transaction* txn) {
        ...
        if (log_manager_ != nullptr) {
            ...
            // CRITICAL: Block until the background thread writes this to the physical file!
            log_manager_->Flush();
        }

        txn->SetState(TransactionState::COMMITTED);

        // Inside TransactionManager::Commit()
        log_manager_->Flush();
        txn->SetState(TransactionState::COMMITTED);
    ...
    void TransactionManager::Abort(Transaction* txn) {
        ...
        for (auto it = index_write_set->rbegin(); it != index_write_set->rend(); ++it) {
            auto& record = *it;
        ...
        for (auto it = write_set->rbegin(); it != write_set->rend(); ++it) {
            ...
            if (record.wtype_ == WType::INSERT) {
                ...
                record.table_heap_->RollbackDelete(record.tuple_.GetRid(), record.tuple_, txn);
            }
        }
