...
namespace tetodb {
    ...
    InsertExecutor::InsertExecutor(ExecutionContext* exec_ctx, const InsertPlanNode* plan)
    ...
    bool InsertExecutor::Next(Tuple* tuple, RID* rid) {
        ...
        if (cursor_ >= raw_exprs.size()) return false;
        ...
        for (const auto* expr : raw_exprs[cursor_]) {
            raw_values.push_back(expr->Evaluate(nullptr, schema, exec_ctx_->GetParams()));
        }
        ...
        for (const auto& fk : table_info_->foreign_keys_) {
            ...
            if (!parent_meta) throw std::runtime_error("Foreign Key Error: Parent table no longer exists.");
            ...
            for (auto* idx : parent_indexes) {
                if (idx->key_attrs_ == fk.parent_key_attrs_) { pk_index = idx; break; }
            }

            if (pk_index) {
                std::vector<Column> p_key_cols = { parent_meta->schema_.GetColumn(fk.parent_key_attrs_[0]) };
                ...
                while (p_iter != parent_meta->table_->End()) {
                    ...
                    if (parent_meta->table_->GetTuple(p_iter.GetRid(), &p_tuple, txn)) {
                        Value p_val = p_tuple.GetValue(&parent_meta->schema_, fk.parent_key_attrs_[0]);
                        if (p_val.CompareEquals(child_val)) { found_parent = true; break; }
                    }
                    ++p_iter;
                }
            }
            ...
            if (!found_parent) {
                throw std::runtime_error("Constraint Violation: Foreign key '" + fk.fk_name_ +
                    "' referenced value does not exist in parent table '" + parent_meta->name_ + "'.");
            }
        }
        ...
        if (table_info_->table_->InsertTuple(to_insert, &new_rid, txn)) {
            ...
            if (!locked) throw std::runtime_error("Transaction Aborted: Failed to acquire Exclusive Lock on Insert.");

            TableWriteRecord write_record(new_rid, WType::INSERT, table_info_->table_.get());
            ...
            for (IndexMetadata* index_info : table_indexes_) {
                ...
                IndexWriteRecord idx_record(new_rid, WType::INSERT, key_tuple, index_info->index_.get());
    ...
    const Schema* InsertExecutor::GetOutputSchema() { return &table_info_->schema_; }
