...
namespace tetodb {
    ...
    SeqScanExecutor::SeqScanExecutor(ExecutionContext* exec_ctx, const SeqScanPlanNode* plan)
        : AbstractExecutor(exec_ctx), plan_(plan) {
    }

    void SeqScanExecutor::Init() {
        metadata_ = exec_ctx_->GetCatalog()->GetTable(plan_->GetTableOid()); // O(1) Lookup!
            ...
            *rid = (*iter_).GetRid();
            ...
            if (txn->GetIsolationLevel() != IsolationLevel::READ_UNCOMMITTED) {
                ...
                if (!locked) {
                    ...
                    throw std::runtime_error("Transaction Aborted: Failed to acquire Shared Lock.");
                }
            }
            ...
            // 4. Advance the iterator NOW so `continue` statements don't cause infinite loops!
            ++(*iter_);
