...
ExecutionEngine::CreateExecutor(const AbstractPlanNode *plan,
  ...
  case PlanType::Projection: {
    ...
    return std::make_unique<ProjectionExecutor>(exec_ctx, proj_plan,
                                                std::move(child));
  }
  case PlanType::HashJoin: {
    ...
    return std::make_unique<HashJoinExecutor>(
        exec_ctx, hj_plan, std::move(left), std::move(right));
  }
  case PlanType::NestedLoopJoin: {
    ...
    return std::make_unique<NestedLoopJoinExecutor>(
        exec_ctx, nlj_plan, std::move(left), std::move(right));
  }
  ...
  case PlanType::Update: {
    ...
    return std::make_unique<UpdateExecutor>(exec_ctx, update_plan,
                                            std::move(child));
  }
  case PlanType::Delete: {
    ...
    return std::make_unique<DeleteExecutor>(exec_ctx, delete_plan,
                                            std::move(child));
  }
