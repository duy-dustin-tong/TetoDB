...
namespace tetodb {
    ...
    INDEX_TEMPLATE_ARGUMENTS
        BPLUSTREE_TYPE::BPlusTree(std::string name, BufferPoolManager* buffer_pool_manager, const KeyComparator& comparator,
            uint32_t leaf_max_size, uint32_t internal_max_size, page_id_t root_page_id)
    ...
    bool BPLUSTREE_TYPE::GetValue(const KeyType& key, std::vector<ValueType>* result, Transaction* transaction) {
        try {
            ...
            while (true) {
                while (idx < leaf_page->GetSize() && comparator_(leaf_page->KeyAt(idx), key) == 0) {
                    result->push_back(leaf_page->ValueAt(idx));
                    found = true;
                    idx++;
                }

                // If we hit the end of this page, duplicates might continue on the next page
                if (idx == leaf_page->GetSize() && leaf_page->GetNextPageId() != INVALID_PAGE_ID) {
                    ...
                    page = next_page;
                    leaf_page = reinterpret_cast<BPlusTreeLeafPage<KeyType, ValueType, KeyComparator>*>(page->GetData());
    ...
    INDEX_TEMPLATE_ARGUMENTS
        ...
        if (page == nullptr) {
            if (op == Operation::READ) root_latch_.RUnlock();
            else root_latch_.WUnlock();
        ...
        while (!node->IsLeafPage()) {
            ...
            if (child_page == nullptr) {
                if (op == Operation::READ) page->RUnlatch();
                else {
                    page->WUnlatch();
                    UnlockUnpinPages(transaction);
            ...
            if (op == Operation::READ) {
                ...
                if (transaction != nullptr) {
                    ...
                    // Safe fallback for single-threaded boot operations to prevent pin leaks
                    page->WUnlatch();
                    buffer_pool_manager_->UnpinPage(page->GetPageId(), false);
                }
            }
    ...
    INDEX_TEMPLATE_ARGUMENTS
        bool BPLUSTREE_TYPE::Remove(const KeyType& key, const ValueType& value, Transaction* transaction) {
        try {
            if (transaction != nullptr) {
                uint32_t target_size = depth_ + 2;
                if (transaction->GetPageSet()->capacity() < target_size) {
    ...
    INDEX_TEMPLATE_ARGUMENTS
        ...
        try {
            ...
            if (new_size > leaf_max_size_) {
                BPlusTreeLeafPage<KeyType, ValueType, KeyComparator>* new_leaf = Split(leaf);
    ...
    INDEX_TEMPLATE_ARGUMENTS
        bool BPLUSTREE_TYPE::RemoveFromLeaf(const KeyType& key, const ValueType& value, Transaction* transaction) {
    ...
    INDEX_TEMPLATE_ARGUMENTS
        ...
        if (old_node->IsRootPage()) {
            ...
            if (page == nullptr) throw Exception(ExceptionType::OUT_OF_MEMORY, "Out of memory");
        ...
        try {
            ...
            if (parent->GetSize() > internal_max_size_) {
                BPlusTreeInternalPage<KeyType, page_id_t, KeyComparator>* new_parent = Split(parent);
    ...
    INDEX_TEMPLATE_ARGUMENTS
        ...
        while (!q.empty()) {
            ...
            std::cout << "[PID=" << pid << " Type=" << (node->IsLeafPage() ? "L" : "I") << " Size=" << node->GetSize() << "] ";
