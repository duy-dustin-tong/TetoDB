...
Tuple::Tuple(const std::vector<Value> &values, const Schema *schema) {
  ...
  for (uint32_t i = 0; i < values.size(); i++) {
    ...
    if (col.IsInlined()) {
      type_handler->SerializeTo(val, header_ptr + col.GetOffset(),
                                col.GetFixedLength());
    } else {
      std::memcpy(header_ptr + col.GetOffset(), &current_heap_offset,
                  sizeof(uint32_t));
      uint32_t written = type_handler->SerializeToHeap(
          val, data_.data() + current_heap_offset);
...
Value Tuple::GetValue(const Schema *schema, uint32_t col_idx) const {
  ...
  if (col.IsInlined()) {
    ...
    uint32_t var_offset;
    std::memcpy(&var_offset, ptr, sizeof(uint32_t));
