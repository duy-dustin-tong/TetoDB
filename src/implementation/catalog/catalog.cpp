...
namespace tetodb {
    ...
    bool Catalog::CreateTable(const std::string& table_name, const Schema& schema,
        ...
        for (const auto& pair : tables_) {
            if (pair.second->name_ == table_name) return false;
        }
        ...
        for (const auto& fk_ast : fk_defs) {
            ...
            if (!parent_meta) {
                throw std::runtime_error("Catalog Error: Parent table '" + fk_ast.parent_table_ + "' does not exist.");
            }

            uint32_t child_col_idx = table_meta->schema_.GetColIdx(fk_ast.child_col_);
            if (child_col_idx == static_cast<uint32_t>(-1)) {
                throw std::runtime_error("Catalog Error: Column '" + fk_ast.child_col_ + "' not found in table '" + table_name + "'.");
            }

            uint32_t parent_col_idx = parent_meta->schema_.GetColIdx(fk_ast.parent_col_);
            if (parent_col_idx == static_cast<uint32_t>(-1)) {
                throw std::runtime_error("Catalog Error: Column '" + fk_ast.parent_col_ + "' not found in parent table.");
            }
            ...
            ForeignKey fk(fk_name, child_attrs, parent_meta->oid_, parent_attrs, fk_ast.on_delete_, fk_ast.on_update_);
    ...
    IndexMetadata* Catalog::CreateIndex(const std::string& index_name,
        ...
        Schema* key_schema = new Schema(key_cols);
        ...
        if (key_size <= 4) {
            ...
            uint32_t leaf_max = (PAGE_SIZE - 28) / (sizeof(GenericKey<4>) + sizeof(RID)) - 1;
            uint32_t internal_max = (PAGE_SIZE - 24) / (sizeof(GenericKey<4>) + sizeof(page_id_t)) - 1;
            index = std::make_unique<BPlusTreeIndex<GenericKey<4>, RID, GenericComparator<4>>>(
                index_name, bpm_, comparator, leaf_max, internal_max, key_schema, root_page_id);
        }
        else if (key_size <= 8) {
            GenericComparator<8> comparator(key_type);
            uint32_t leaf_max = (PAGE_SIZE - 28) / (sizeof(GenericKey<8>) + sizeof(RID)) - 1;
            uint32_t internal_max = (PAGE_SIZE - 24) / (sizeof(GenericKey<8>) + sizeof(page_id_t)) - 1;
            index = std::make_unique<BPlusTreeIndex<GenericKey<8>, RID, GenericComparator<8>>>(
                index_name, bpm_, comparator, leaf_max, internal_max, key_schema, root_page_id);
        }
        else if (key_size <= 16) {
            GenericComparator<16> comparator(key_type);
            uint32_t leaf_max = (PAGE_SIZE - 28) / (sizeof(GenericKey<16>) + sizeof(RID)) - 1;
            uint32_t internal_max = (PAGE_SIZE - 24) / (sizeof(GenericKey<16>) + sizeof(page_id_t)) - 1;
            index = std::make_unique<BPlusTreeIndex<GenericKey<16>, RID, GenericComparator<16>>>(
                index_name, bpm_, comparator, leaf_max, internal_max, key_schema, root_page_id);
        }
        else if (key_size <= 32) {
            GenericComparator<32> comparator(key_type);
            uint32_t leaf_max = (PAGE_SIZE - 28) / (sizeof(GenericKey<32>) + sizeof(RID)) - 1;
            uint32_t internal_max = (PAGE_SIZE - 24) / (sizeof(GenericKey<32>) + sizeof(page_id_t)) - 1;
            index = std::make_unique<BPlusTreeIndex<GenericKey<32>, RID, GenericComparator<32>>>(
                index_name, bpm_, comparator, leaf_max, internal_max, key_schema, root_page_id);
        }
        else if (key_size <= 64) {
            GenericComparator<64> comparator(key_type);
            uint32_t leaf_max = (PAGE_SIZE - 28) / (sizeof(GenericKey<64>) + sizeof(RID)) - 1;
            uint32_t internal_max = (PAGE_SIZE - 24) / (sizeof(GenericKey<64>) + sizeof(page_id_t)) - 1;
            index = std::make_unique<BPlusTreeIndex<GenericKey<64>, RID, GenericComparator<64>>>(
                index_name, bpm_, comparator, leaf_max, internal_max, key_schema, root_page_id);
        }
        else {
            throw std::runtime_error("Index key size exceeds maximum supported 64 bytes!");
        }
        ...
        auto index_meta = std::make_unique<IndexMetadata>(
            index_name, table_meta->oid_, std::move(index), oid, key_attrs, is_unique
        );
    ...
    IndexMetadata* Catalog::CreateIndex(const std::string& index_name,
        ...
        for (const auto& col_name : column_names) {
            ...
            if (col_idx == static_cast<uint32_t>(-1)) {
                throw std::runtime_error("Catalog Error: Column '" + col_name + "' not found.");
            }
            key_attrs.push_back(col_idx);
        }

        // --- UPDATED: Pass is_unique to base CreateIndex ---
        IndexMetadata* result = CreateIndex(index_name, table_meta->oid_, key_attrs, is_unique, txn);
    ...
    void Catalog::PopulateIndex(TableMetadata* table_meta, IndexMetadata* index_meta, Transaction* txn) {
    ...
    bool Catalog::DropTable(const std::string& table_name) {
        ...
        for (const auto& [other_oid, other_meta] : tables_) {
            if (other_oid == table_oid) continue;

            for (const auto& fk : other_meta->foreign_keys_) {
                if (fk.parent_table_oid_ == table_oid) {
                    throw std::runtime_error("Execution Error: Cannot drop table '" + table_name +
                        "'. It is referenced by a foreign key in table '" + other_meta->name_ + "'.");
                }
            }
        }
    ...
    void Catalog::SaveCatalog(const std::string& file_path) {
        ...
        for (const auto& [oid, meta] : tables_) {
            ...
            for (const auto& col : meta->schema_.GetColumns()) {
                out << "COLUMN " << col.GetName() << " " << static_cast<int>(col.GetTypeId()) << "\n";
            }
            out << "END\n";
        }
            ...
            for (const auto& fk : meta->foreign_keys_) {
                ...
                if (!parent_name.empty()) {
                    out << "FK " << meta->name_ << " " << fk.fk_name_ << " " << parent_name << " "
                        ...
                        << static_cast<int>(fk.on_delete_) << " " << static_cast<int>(fk.on_update_) << "\n";
                }
            }
        }
    }
        ...
        while (std::getline(in, line)) {
            ...
            if (token == "TABLE") {
                ...
                if (t_meta) {
                    ...
                    CreateIndex(idx_name, t_meta->oid_, attrs, is_unique, nullptr, root_page_id);
                }
            }
            else if (token == "FK") {
                ...
                ss >> child_table >> fk_name >> parent_table >> child_col_idx >> parent_col_idx >> on_delete_int >> on_update_int;
                ...
                CreateTable(current_table, schema, current_first_page_id, std::vector<uint32_t>{}, std::vector<ForeignKeyDef>{});
            }
        }
    }
