...
namespace tetodb {
    ...
    void RecoveryManager::Redo() {
        ...
        std::cout << "[RECOVERY] Phase 1 & 2: Analysis and Redo Started..." << std::endl;
        uint32_t offset = 0;

        while (true) {
            // 1. Peek at the first 4 bytes (the size of the record)
            uint32_t record_size;
            if (!log_file.read(reinterpret_cast<char*>(&record_size), sizeof(uint32_t))) {
                break; // Clean EOF
            }
            ...
            if (record_size < 20 || record_size > PAGE_SIZE * 2) {
                std::cout << "[RECOVERY] Encountered partial/corrupted log record at end of file. Halting Analysis." << std::endl;
                break;
            }
            ...
            if (!log_file.read(buffer.data() + sizeof(uint32_t), record_size - sizeof(uint32_t))) {
                std::cout << "[RECOVERY] Log payload truncated. Halting Analysis." << std::endl;
                break;
            }
            ...
            if (type == LogRecordType::NEWPAGE) {
                ...
                if (page != nullptr) {
                    ...
                    if (table_page->GetNextPageId() == 0) {
                        ...
                        if (rid.GetPageId() > 0) {
                            Page* prev_page = bpm_->FetchPage(rid.GetPageId() - 1);
                            if (prev_page != nullptr) {
                                WritePageGuard prev_guard(bpm_, prev_page);
                                prev_guard.As<TablePage>()->SetNextPageId(rid.GetPageId());
        ...
        std::cout << "[RECOVERY] Found " << active_txn_.size() << " active (loser) transactions." << std::endl;
    }

    void RecoveryManager::Undo() {
        if (active_txn_.empty()) {
            std::cout << "[RECOVERY] No active transactions to undo. Database is clean." << std::endl;
            return;
        }
        ...
        for (auto const& [txn_id, last_lsn] : active_txn_) {
            ...
            std::cout << " -> Rolling back incomplete Transaction " << txn_id << std::endl;

            while (current_lsn != INVALID_LSN) {
                auto offset_it = lsn_mapping_.find(current_lsn);
                if (offset_it == lsn_mapping_.end()) {
                    std::cerr << "[RECOVERY ERROR] Missing LSN " << current_lsn << " in mapping!" << std::endl;
                    break;
                }
                ...
                std::vector<char> buffer(record_size);
                std::memcpy(buffer.data(), &record_size, sizeof(uint32_t));
                log_file.read(buffer.data() + sizeof(uint32_t), record_size - sizeof(uint32_t));
                ...
                std::cout << "[UNDO TRACE] LSN: " << current_lsn
                    ...
                    << " | Target: Page " << rid.GetPageId()
                    << ", Slot " << rid.GetSlotId() << std::endl;
        ...
        log_file.close();
        std::cout << "[RECOVERY] Undo Complete. Database is fully ACID compliant and ready for queries!" << std::endl;
    }
