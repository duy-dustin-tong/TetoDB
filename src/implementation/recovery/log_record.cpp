...
// SIZE CALCULATION
// ==========================================
uint32_t LogRecord::CalculateSize(LogRecordType type, const Tuple &old_tuple,
                                  const Tuple &new_tuple) {
  // Standard Header Size: size (4) + lsn (4) + txn_id (4) + prev_lsn (4) + type
  // (4) + is_clr (4) + undo_next_lsn (4) = 28 bytes
  uint32_t size = 28;
  ...
  case LogRecordType::ABORT:
    // No payload! Just the 28-byte header.
    break;
...
uint32_t LogRecord::Serialize(char *dest) const {
  ...
  // 1. Write Header
  std::memcpy(dest + offset, &size_, sizeof(uint32_t));
  ...
  offset += sizeof(lsn_t);
  std::memcpy(dest + offset, &log_record_type_, sizeof(LogRecordType));
  offset += sizeof(LogRecordType);
  std::memcpy(dest + offset, &is_clr_, sizeof(uint32_t));
  offset += sizeof(uint32_t);
  std::memcpy(dest + offset, &undo_next_lsn_, sizeof(lsn_t));
  offset += sizeof(lsn_t);
  ...
  if (log_record_type_ == LogRecordType::INSERT ||
    ...
    std::memcpy(dest + offset, &target_rid_, sizeof(RID));
    offset += sizeof(RID);

    // WRITE THE SIZE PREFIX FIRST
    uint32_t tup_size = new_tuple_.GetSize();
    std::memcpy(dest + offset, &tup_size, sizeof(uint32_t));
    offset += sizeof(uint32_t);
    ...
    std::memcpy(dest + offset, &old_size, sizeof(uint32_t));
    offset += sizeof(uint32_t);
    ...
    std::memcpy(dest + offset, &new_size, sizeof(uint32_t));
    offset += sizeof(uint32_t);
    new_tuple_.SerializeTo(dest + offset);
    offset += new_size;
    ...
    std::memcpy(dest + offset, &target_rid_, sizeof(RID));
    offset += sizeof(RID);
    std::memcpy(dest + offset, &prev_page_id_, sizeof(page_id_t));
...
uint32_t LogRecord::Deserialize(const char *src) {
  ...
  // 1. Read Header
  std::memcpy(&size_, src + offset, sizeof(uint32_t));
  offset += sizeof(uint32_t);
  std::memcpy(&lsn_, src + offset, sizeof(lsn_t));
  offset += sizeof(lsn_t);
  std::memcpy(&txn_id_, src + offset, sizeof(txn_id_t));
  offset += sizeof(txn_id_t);
  std::memcpy(&prev_lsn_, src + offset, sizeof(lsn_t));
  offset += sizeof(lsn_t);
  std::memcpy(&log_record_type_, src + offset, sizeof(LogRecordType));
  offset += sizeof(LogRecordType);
  std::memcpy(&is_clr_, src + offset, sizeof(uint32_t));
  offset += sizeof(uint32_t);
  std::memcpy(&undo_next_lsn_, src + offset, sizeof(lsn_t));
  offset += sizeof(lsn_t);
  ...
  if (log_record_type_ == LogRecordType::INSERT ||
    ...
    std::memcpy(&target_rid_, src + offset, sizeof(RID));
    offset += sizeof(RID);

    // READ THE SIZE PREFIX FIRST
    uint32_t tup_size = 0;
    std::memcpy(&tup_size, src + offset, sizeof(uint32_t));
    offset += sizeof(uint32_t);
    ...
    // OLD TUPLE
    uint32_t old_size = 0;
    std::memcpy(&old_size, src + offset, sizeof(uint32_t));
    offset += sizeof(uint32_t);
    ...
    // NEW TUPLE
    uint32_t new_size = 0;
    std::memcpy(&new_size, src + offset, sizeof(uint32_t));
    offset += sizeof(uint32_t);
    ...
    std::memcpy(&target_rid_, src + offset, sizeof(RID));
    offset += sizeof(RID);
    std::memcpy(&prev_page_id_, src + offset, sizeof(page_id_t));
    offset += sizeof(page_id_t);
  }
