...
namespace tetodb {
    ...
    const AbstractPlanNode* Planner::PlanQuery(const ASTNode* ast) {
        if (!ast) throw std::runtime_error("Planner Error: AST is null");
    ...
    const AbstractPlanNode* Planner::PlanSelect(const SelectStatement* stmt) {
        ...
        if (!left_meta) throw std::runtime_error("Planner Error: Table '" + left_table_name + "' not found.");

        // --- STORE LEFT ALIAS CONTEXT ---
        active_left_alias_ = stmt->from_table_->alias_.empty() ? left_table_name : stmt->from_table_->alias_;
        active_left_schema_ = &left_meta->schema_;
        active_right_alias_ = "";
        active_right_schema_ = nullptr;


        auto left_scan = std::make_unique<SeqScanPlanNode>(&left_meta->schema_, left_meta->oid_);
        ...
        if (!stmt->joins_.empty()) {
            ...
            if (!right_meta) throw std::runtime_error("Planner Error: Table '" + right_table_name + "' not found.");

            // --- STORE RIGHT ALIAS CONTEXT ---
            active_right_alias_ = join_ast->right_table_->alias_.empty() ? right_table_name : join_ast->right_table_->alias_;
            active_right_schema_ = &right_meta->schema_;

            auto right_scan = std::make_unique<SeqScanPlanNode>(&right_meta->schema_, right_meta->oid_);
            ...
            if (join_ast->condition_->type_ != ASTNodeType::BINARY_EXPR) {
                throw std::runtime_error("Planner Error: JOIN condition must be a binary expression");
            }
            const auto* bin_expr = static_cast<const BinaryExpr*>(join_ast->condition_.get());

            const auto* left_col_ast = static_cast<const ColumnRefExpr*>(bin_expr->left_.get());
            const auto* right_col_ast = static_cast<const ColumnRefExpr*>(bin_expr->right_.get());

            uint32_t left_col_idx = left_meta->schema_.GetColIdx(left_col_ast->col_name_);
            uint32_t right_col_idx = right_meta->schema_.GetColIdx(right_col_ast->col_name_);

            auto left_key_expr = std::make_unique<ColumnValueExpression>(0, left_col_idx);
            auto right_key_expr = std::make_unique<ColumnValueExpression>(1, right_col_idx);

            static const std::unordered_map<std::string, CompType> comp_map = {
                {"=", CompType::EQUAL}, {"<", CompType::LESS_THAN}, {">", CompType::GREATER_THAN},
                {"<=", CompType::LESS_THAN_OR_EQUAL}, {">=", CompType::GREATER_THAN_OR_EQUAL}, {"!=", CompType::NOT_EQUAL}
            };
            auto comp_it = comp_map.find(bin_expr->op_);
            if (comp_it == comp_map.end()) throw std::runtime_error("Planner Error: Unsupported JOIN operator '" + bin_expr->op_ + "'");

            auto comp_expr = std::make_unique<ComparisonExpression>(comp_it->second, std::move(left_key_expr), std::move(right_key_expr));
            ...
            auto nlj = std::make_unique<NestedLoopJoinPlanNode>(current_schema, current_plan, right_scan_ptr, predicate_ptr);
        ...
        if (stmt->where_clause_) {
            auto filter_expr = PlanExpression(stmt->where_clause_.get(), current_schema);
            const AbstractExpression* predicate_ptr = filter_expr.get();
            expressions_.push_back(std::move(filter_expr));

            auto filter_plan = std::make_unique<FilterPlanNode>(current_schema, current_plan, predicate_ptr);
        ...
        for (const auto& expr_ast : stmt->select_list_) {
            if (expr_ast->type_ == ASTNodeType::AGGREGATE) has_aggregation = true;
        }
        ...
        if (has_aggregation) {
            ...
            for (const auto& gb_ast : stmt->group_bys_) {
                ...
                if (gb_ast->type_ == ASTNodeType::COLUMN_REF) {
                    std::string col_name = static_cast<const ColumnRefExpr*>(gb_ast.get())->col_name_;
            ...
            static const std::unordered_map<std::string, AggregationType> agg_map = {
                {"COUNT", AggregationType::COUNT_STAR}, {"SUM", AggregationType::SUM},
                {"MIN", AggregationType::MIN}, {"MAX", AggregationType::MAX},
                {"AVG", AggregationType::AVERAGE}, {"AVERAGE", AggregationType::AVERAGE},
                {"MED", AggregationType::MEDIAN}, {"MEDIAN", AggregationType::MEDIAN}
            };

            for (const auto& expr_ast : stmt->select_list_) {
                if (expr_ast->type_ == ASTNodeType::AGGREGATE) {
                    const auto* agg_ast = static_cast<const AggregateExpr*>(expr_ast.get());
                    auto arg_expr = PlanExpression(agg_ast->arg_.get(), current_schema);

                    auto it = agg_map.find(agg_ast->func_name_);
                    if (it == agg_map.end()) throw std::runtime_error("Planner Error: Unknown aggregate function: " + agg_ast->func_name_);

                    AggregationType atype = it->second;
                    TypeId return_type = (atype == AggregationType::AVERAGE || atype == AggregationType::MEDIAN) ? TypeId::DECIMAL : TypeId::INTEGER;
            ...
            auto agg_plan = std::make_unique<AggregationPlanNode>(current_schema, current_plan, std::move(group_bys), std::move(aggregates), std::move(agg_types));
        ...
        for (const auto& expr_ast : stmt->select_list_) {
            if (expr_ast->type_ == ASTNodeType::COLUMN_REF) {
                ...
                if (!col_ref->alias_.empty()) alias_map[col_ref->alias_] = col_ref->col_name_;
            }
            else if (expr_ast->type_ == ASTNodeType::AGGREGATE) {
                const auto* agg_expr = static_cast<const AggregateExpr*>(expr_ast.get());
                if (!agg_expr->alias_.empty()) alias_map[agg_expr->alias_] = agg_expr->func_name_;
            }
        }
        ...
        if (!stmt->order_bys_.empty()) {
            ...
            auto sort_plan = std::make_unique<SortPlanNode>(current_schema, current_plan, std::move(order_bys));
        ...
        for (const auto& expr_ast : stmt->select_list_) {
            ...
            if (expr_ast->type_ == ASTNodeType::COLUMN_REF) {
                ...
                if (!col_ref->table_alias_.empty()) {
                    if (col_ref->table_alias_ == active_right_alias_ && active_right_schema_) {
                    ...
                    else if (col_ref->table_alias_ == active_left_alias_ && active_left_schema_) {
                ...
                throw std::runtime_error("Planner Error: Only Columns and Aggregates supported in SELECT list");
            }
            ...
            if (col_idx == static_cast<uint32_t>(-1)) throw std::runtime_error("Planner Error: Column '" + target_col + "' not found.");

            std::string final_name = alias.empty() ? target_col : alias;
            out_columns.push_back(Column(final_name, current_schema->GetColumn(col_idx).GetTypeId()));
        ...
        auto proj_plan = std::make_unique<ProjectionPlanNode>(out_schema_ptr, current_plan, proj_exprs);
        ...
        if (stmt->limit_count_ != -1 || stmt->offset_count_ > 0) {
            auto limit_plan = std::make_unique<LimitPlanNode>(out_schema_ptr, current_root, stmt->limit_count_, stmt->offset_count_);
    ...
    const AbstractPlanNode* Planner::PlanInsert(const InsertStatement* stmt) {
        ...
        if (!table_meta) throw std::runtime_error("Planner Error: Table '" + table_name + "' not found.");
        ...
        for (const auto& row_exprs : stmt->values_) {
            if (row_exprs.size() != schema.GetColumnCount()) throw std::runtime_error("Planner Error: INSERT values count does not match schema.");
        ...
        auto insert_plan = std::make_unique<InsertPlanNode>(table_meta->oid_, std::move(insert_exprs));
    ...
    const AbstractPlanNode* Planner::PlanUpdate(const UpdateStatement* stmt) {
        ...
        const Schema& schema = table_meta->schema_;
        auto child_scan = std::make_unique<SeqScanPlanNode>(&schema, table_meta->oid_);
        ...
        if (stmt->where_clause_) {
            ...
            auto filter_plan = std::make_unique<FilterPlanNode>(&schema, current_plan, predicate_ptr);
            current_plan = filter_plan.get();
            plan_nodes_.push_back(std::move(filter_plan));
        }
        ...
        for (const auto& set_clause : stmt->set_clauses_) {
            ...
            if (col_idx == static_cast<uint32_t>(-1)) throw std::runtime_error("Planner Error: Column '" + col_name + "' not found.");

            const auto* ast_const = dynamic_cast<const ConstantExpr*>(set_clause.second.get());
            if (!ast_const) throw std::runtime_error("Planner Error: UPDATE SET only supports raw constants right now.");

            TypeId expected_type = schema.GetColumn(col_idx).GetTypeId();
            std::string raw_str = ast_const->value_;
            Value val;

            if (expected_type == TypeId::INTEGER) val = Value(TypeId::INTEGER, std::stoi(raw_str));
            else if (expected_type == TypeId::VARCHAR) {
                if (raw_str.length() >= 2 && raw_str.front() == '\'' && raw_str.back() == '\'') {
        ...
        auto update_plan = std::make_unique<UpdatePlanNode>(&schema, current_plan, table_meta->oid_, std::move(update_exprs));
    ...
    const AbstractPlanNode* Planner::PlanDelete(const DeleteStatement* stmt) {
        ...
        auto child_scan = std::make_unique<SeqScanPlanNode>(&table_meta->schema_, table_meta->oid_);
        const AbstractPlanNode* current_plan = child_scan.get();
        plan_nodes_.push_back(std::move(child_scan));

        if (stmt->where_clause_) {
            auto filter_expr = PlanExpression(stmt->where_clause_.get(), &table_meta->schema_);
            const AbstractExpression* predicate_ptr = filter_expr.get();
            expressions_.push_back(std::move(filter_expr));

            auto filter_plan = std::make_unique<FilterPlanNode>(&table_meta->schema_, current_plan, predicate_ptr);
            current_plan = filter_plan.get();
            plan_nodes_.push_back(std::move(filter_plan));
        }

        auto delete_plan = std::make_unique<DeletePlanNode>(current_plan, table_meta->oid_);
    ...
    std::unique_ptr<AbstractExpression> Planner::PlanExpression(const Expr* ast_expr, const Schema* schema, const std::unordered_map<std::string, std::string>& alias_map) {
        ...
        if (const auto* col_ref = dynamic_cast<const ColumnRefExpr*>(ast_expr)) {
            ...
            if (col_idx == static_cast<uint32_t>(-1)) {
                throw std::runtime_error("Planner Error: Column/Alias '" + col_ref->col_name_ + "' not found.");
            }
        ...
        if (const auto* const_expr = dynamic_cast<const ConstantExpr*>(ast_expr)) {
            ...
            if (raw_str.length() >= 2 && raw_str.front() == '\'' && raw_str.back() == '\'') {
                ...
                try { val = Value(TypeId::INTEGER, std::stoi(raw_str)); }
                catch (...) { val = Value(TypeId::VARCHAR, raw_str); }
            }
        ...
        if (const auto* agg_expr = dynamic_cast<const AggregateExpr*>(ast_expr)) {
            ...
            if (col_idx == static_cast<uint32_t>(-1)) throw std::runtime_error("Planner Error: Aggregate '" + agg_expr->func_name_ + "' not found in schema.");
            return std::make_unique<ColumnValueExpression>(0, col_idx);
        }
        ...
        if (const auto* bin_expr = dynamic_cast<const BinaryExpr*>(ast_expr)) {
            ...
            if (it == comp_map.end()) throw std::runtime_error("Planner Error: Unsupported operator '" + bin_expr->op_ + "'");

            return std::make_unique<ComparisonExpression>(it->second, std::move(left), std::move(right));
        }
